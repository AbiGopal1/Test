...
col shipment_details noprint
col shipment_marks noprint
col shipment_period noprint
col tracer_code noprint
Col ADDRESS4 NoPrint
Col FROm_PLACE NoPrint
Col TO_PLACE NoPrint
SPOOL G:\JUNK\SPOOL.SPL

--ALTER SESSION SET CURRENT_SCHEMA = CBSPRODUBS;
...
PROMPT lctbs_amnd_vals FCC 5.1 tables
SELECT * FROM lctbs_amnd_vals
WHERE contract_ref_no=UPPER('&CRN')
/

PROMPT CSTBS_MULTIAUTH_OVD_DETAIL;
select * from CSTBS_MULTIAUTH_OVD_DETAIL where contract_ref_no  = UPPER('&CRN')
/

PROMPT cstm_product_status_master;
select * from cstm_product_status_master where product =SUBSTR( UPPER('&CRN'),4,4)
/

PROMPT ldtbs_contract_linkages;
select * from ldtbs_contract_linkages where contract_ref_no  = UPPER('&CRN')
/

PROMPT td_lien_mast;
select * from cbsPRODFCR.td_lien_mast  where COD_ACCT_NO_BENEF = UPPER('&CRN')
/
